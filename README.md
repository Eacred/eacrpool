...
* The `eacrpool` executable will be installed to `$GOPATH/bin`.  `GOPATH`
...
Eacrpool requires [dcrd](https://https://github.com/eacred/dcrd) and [eacrwallet](https://https://github.com/Eacred/eacrwallet) when configured as a mining pool, it only requires dcrd when configured as a solo pool. 
Deploying the user interface requires copying the `eacrpool/gui/assets` folder from 
...
Eacrpool is ideal for miners running medium-to-large mining operations. The 
...
the cost of running a pool. It will most likely not be cost effective to run eacrpool for a small mining operation, the better option here would be using  
a public mining pool instead. 

For people looking to setup a publicly available mining pool, eacrpool's well-documented configuration and simple setup process also make it
 a great option.
...
The [integrated github issue tracker](https://github.com/Eacred/eacrpool/issues)
is used for this project.

## License

eacrpool is licensed under the [copyfree](http://copyfree.org) ISC License.
